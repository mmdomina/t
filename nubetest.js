...
(async () => {
  ...
  console.log('\n11. El que sólo tiene el código no puede ser otro');
  /* El código de seis letras se dicta en voz alta en el tee y se manda por
     WhatsApp: hay que asumir que lo tiene cualquiera. Lo que no puede tener
     es la llave de otro teléfono. */
  const laLlave = await A.p.evaluate(() => RONDA.llave);
  ok(typeof laLlave === 'string' && laLlave.length > 20, 'cada teléfono guarda su propia llave');
  const suLlave = await B.p.evaluate(() => RONDA.llave);
  ...
  ok(yo && yo.nombre === 'Mauro Domina' && yo.hcp_cancha !== 54,
     'el jugador quedó como estaba: ' + (yo && yo.nombre));
  ok(despues.anotaciones.filter(a => a.de === idVictima).length === antesDelAtaque,
     'y no le entró ninguna anotación falsa');
  ...
  await A.p.evaluate(() => latir());
  await esperar(900);
  const conElAtaque = await A.p.evaluate(() => {
    S.tab = 'arranque'; render();
    const s = document.getElementById('screen');
    return { roto: !!window.__roto, imgs: s.querySelectorAll('img').length,
             texto: s.innerText.includes('<img src=x') };
  });
  ok(!conElAtaque.roto, 'un nombre con HTML NO ejecuta código en el teléfono del grupo');
  ok(conElAtaque.imgs === 0, 'y no inyecta ningún elemento en la pantalla');
  ok(conElAtaque.texto, 'se ve como texto, literal, que es lo que corresponde');


  console.log('\n12. Borrado lógico: la ronda se borra del servidor');
  /* Borrar no es una función de la app: en el servidor de verdad `anon` no la
     puede llamar. Se dispara desde el panel del test, que hace de SQL Editor. */
  const antesDeBorrar = await A.p.evaluate(() => Object.keys(RONDA.anot).length);
  await fetch(MOCK + '/__test?borrar=' + codigo);
  ok(await hasta(A.p, () => RONDA === null, null, 16000),
     'el teléfono se entera en el próximo latido y sale de la ronda');
  const trasBorrar = await A.p.evaluate(() => ({
    ronda: RONDA, score: R.scores[4],
    pantalla: document.getElementById('screen').innerText.replace(/\s+/g,' ')
  }));
  ok(trasBorrar.score === 4, 'PERO su tarjeta no se toca: el score sigue');
  ok(!/\bnull\b|\bNaN\b|undefined/.test(trasBorrar.pantalla),
     'y la pantalla no queda con huecos');
  console.log(`    tenía ${antesDeBorrar} anotaciones del grupo · ahora está fuera de la ronda`);
