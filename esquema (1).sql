...
--
--  CÓMO SE INSTALA (Neon)
--  1. En el proyecto, habilitar la DATA API. Eso es lo que deja entrar a la
--     app por HTTP, y de paso crea los dos roles que usa: `anonymous` y
--     `authenticated`.
--  2. SQL Editor → pegar todo esto → Run.
--  Si se corre al revés no se rompe nada: el bloque de abajo crea los roles
...
--  Los crea Neon al habilitar la Data API. Este bloque es el seguro para el
--  caso de correr el SQL primero: sin los roles, cada `grant` de abajo
--  cortaría con error y el esquema quedaría a medio instalar.
do $$
begin
...
$$;
revoke execute on function llave_nueva() from public, anonymous, authenticated;
...
revoke execute on function recortar(text,int) from public, anonymous, authenticated;
...
-- sin políticas: entrando sin autenticarse no se llega a las tablas.
-- Neon lo dice con estas palabras: RLS activo y sin políticas = todo bloqueado.

revoke all on rondas, jugadores, anotaciones from anonymous, authenticated;

--  Pero para poder LLAMAR a una función hace falta poder ver el esquema. Esto
--  no abre ninguna tabla: sin `select` ni `insert`, `usage` sola no alcanza
--  para leer ni escribir nada. En Supabase venía puesto de fábrica; en Neon
--  hay que darlo a mano, y sin esto la app recibe un error de permisos.
...
--  concedidas a `anonymous` y serían la misma puerta sin llave que estamos cerrando.
...
grant execute on function ronda_abrir(text,text,text,int,boolean,text,text,text,numeric,int,text,text) to anonymous;
grant execute on function ronda_unirse(text,text,text,text,numeric,int,text,text) to anonymous;
grant execute on function ronda_estado(text,timestamptz)                          to anonymous;
grant execute on function anotar(text,text,text,text,int,int,int,int,int,text)    to anonymous;
grant execute on function marcar_a(text,text,text,text)                           to anonymous;
grant execute on function ronda_cerrar(text,text)                                 to anonymous;

-- ---------- borrar, restaurar, purgar ----------
--  Las tres son de administración: NO se le dan a `anonymous`. Hoy no hay
--  cuentas, así que cualquiera que abra la app es `anonymous`; darle una
...
revoke execute on function borrar_ronda(text)          from public, anonymous, authenticated;
revoke execute on function restaurar_ronda(text)       from public, anonymous, authenticated;
revoke execute on function limpiar_rondas_viejas()     from public, anonymous, authenticated;
revoke execute on function purgar_borradas(int)        from public, anonymous, authenticated;

-- ---------- cierre: que nada quede abierto por omisión ----------
--  Postgres le regala `execute` a `public` en CADA función nueva. Más arriba se
--  lo sacamos una por una, pero si mañana alguien agrega una función y se
--  olvida de ese renglón, nace abierta — y ya pasó una vez, con
--  `limpiar_rondas_viejas()`. Esto lo cierra de raíz: se le quita `execute` a
--  TODAS las funciones del esquema y se vuelven a dar solamente las seis
--  puertas de la app. Volver a correr este archivo entero es seguro: termina
